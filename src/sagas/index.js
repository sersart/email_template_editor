...
import { getCompany, getComponents, getTemplate, saveImage, saveTemplate, sendTestEmail } from './api';

const messageTimeout = 3333;

function* loadCompany(action) {
	try {
		const res = yield getCompany(action.host_name).then(res => res);
        yield put({type: 'GET_COMPANY_SUCCESS', company: res.payload});
        console.log("SAGA readed company", res);
	} catch(e) {
		console.error(e);
...
function* mySagas() {
	...
	yield takeLatest("LOAD_COMPANY", loadCompany);
