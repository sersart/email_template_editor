...
const OptionsFeedbackText = ({ block, t, onPropChange }) => {
	...
	return (
		<div>
			...
			<hr />
			<div>
				<label>{t("Font size")} <input type="number" value={fontSize.match(/\d+/)[0]} onChange={(e) => onPropChange('fontSize', `${e.target.value}px`, true)} /></label>
			</div>
			<div>
				<label>
					{t("Font family")}
...
export default translate('translations')(OptionsFeedbackText);
